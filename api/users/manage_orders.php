...
<head>
    ...
    <style>
        /* (Kept exact same CSS as previous iteration to save space, 
           ensuring visual consistency with your dashboard) */
        :root {
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Sidebar & Topbar Base (Collapsed for brevity, identical to your file) */
        ...
        /* Collapsed Sidebar rules omitted for brevity but behave exactly like your index.php */
        ...
        .page-header p { font-size: 0.85rem; color: var(--fg-muted); }
        ...
        .table-order-id { font-weight: 600; color: var(--red); font-size: 0.84rem; transition: color var(--duration) var(--ease); }
        ...
        .table-items { color: var(--fg-secondary); font-size: 0.84rem; max-width: 250px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: block; }

        .status-badge { display: inline-flex; align-items: center; gap: 6px; font-size: 0.76rem; font-weight: 600; padding: 4px 11px; border-radius: var(--radius-full); white-space: nowrap; }
        ...
        .table-action-btn.track { border-color: rgba(212,148,42,0.2); background: rgba(212,148,42,0.1); color: #D4942A; }
        .table-action-btn.track:hover { background: #D4942A; color: #fff; border-color: #D4942A; }
...
<div class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <!-- USER PERSPECTIVE: Changed to "Purchases & Tracking" -->
        <div class="nav-item" data-dropdown>
            ...
            <div class="dropdown-menu open" role="menu">
                <!-- USER PERSPECTIVE: Clear distinction between active orders and past receipts -->
                <a href="/account/orders" class="active" role="menuitem">My Orders & Tracking</a>
                <a href="/account/invoices" role="menuitem">Past Invoices & Receipts</a>
                <a href="/account/custom-orders" role="menuitem">Custom Hat Requests</a>
...
<div class="main-wrapper">
    ...
    <main class="main-content">
        <!-- USER PERSPECTIVE: Friendly, clear copy -->
        <div class="page-header">
            <div>
                ...
                <p>Follow your latest hats on their journey to you, or view past purchases.</p>
            </div>
        </div>
...
/* ================================================================
   ...
   ================================================================ */
const allOrders = [
    { id: "HH-10024", date: "2025-06-10", status: "in-transit", total: 94.99, items: "Wide Brim Fedora, Silk Scarf" },
    { id: "HH-10023", date: "2025-06-08", status: "delivered", total: 89.00, items: "Summer Straw Boater" },
    { id: "HH-10022", date: "2025-06-05", status: "delivered", total: 115.99, items: "Victorian Top Hat, Feather Accessory" },
    { id: "HH-10021", date: "2025-06-01", status: "processing", total: 98.99, items: "Custom Cloche Hat" },
    { id: "HH-10020", date: "2025-05-28", status: "delivered", total: 84.00, items: "Beret (Navy)" },
    { id: "HH-10019", date: "2025-05-25", status: "delivered", total: 60.99, items: "Trilby Hat (Brown)" }
];
...
function renderOrders() {
    ...
    if (allOrders.length === 0) {
        container.innerHTML = `
            <div class="table-empty">
                ...
                <a href="/shop" class="table-empty-btn"><i class="fa-solid fa-bag-shopping"></i> Start Shopping</a>
            </div>
        `;
        return;
    }
    ...
    container.innerHTML = `
        <table class="data-table">
            ...
            <tbody>
                ${allOrders.map(o => {
                    // USER PERSPECTIVE: Dynamic buttons based on what the user actually cares about
                    let actionBtn = '';
                    if (o.status === 'in-transit') {
                        actionBtn = `<a href="/account/orders?id=${escapeHtml(o.id)}#tracking" class="table-action-btn track"><i class="fa-solid fa-location-dot" style="font-size:0.7rem; margin-right:4px;"></i> Track</a>`;
                    } else if (o.status === 'processing') {
                        actionBtn = `<a href="/account/orders?id=${escapeHtml(o.id)}" class="table-action-btn">Details</a>`;
                    } else {
                        actionBtn = `<a href="/account/orders?id=${escapeHtml(o.id)}" class="table-action-btn">View</a>`;
                    }

                    return `
                    <tr>
                        <td><a href="/account/orders?id=${escapeHtml(o.id)}" class="table-order-id">#${escapeHtml(o.id)}</a></td>
                        <td><span class="table-date">${formatDate(o.date)}</span></td>
                        <td class="hide-mobile"><span class="table-items" title="${escapeHtml(o.items)}">${escapeHtml(o.items)}</span></td>
                        <td>
...
/* ================================================================
   SIDEBAR & TOPBAR LOGIC (Identical to index)
   ================================================================ */
...
if (notifToggle) { notifToggle.addEventListener('click', () => showToast('You have 2 orders in transit and 1 request update.', 'info')); }
